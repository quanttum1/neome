...
export default function OpenTask() {
  ...
  function update(e: React.FormEvent): void {
    ...
    if (!rewardRef.current && (!('version' in task) || task.version == 2)) return setError("Reward is not set");
    if (!penaltyRef.current && (!('version' in task) || task.version == 2)) return setError("Penalty is not set");
